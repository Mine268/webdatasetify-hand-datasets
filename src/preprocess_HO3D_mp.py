...
rmano_pose_mean = np.load("models/mano/mano_right_mean.npy")
...
def prepare_data():
    ...
    for seq in tqdm(annot_seqs, ncols=70):
        ...
        for img_path, annot_path in seq:
            ...
            # joint_cam, joint_rel
            joint_cam = annot["handJoints3D"] * np.array([[1, -1, -1]]) * 1e3
            ...
            if "handPose" in annot:
                mano_pose = annot["handPose"]
                root_pose = annot["handPose"][:3]
                root_pose_mat, _ = cv2.Rodrigues(root_pose)
                root_pose_mat = np.dot(R_x_pi, root_pose_mat)
                root_pose, _ = cv2.Rodrigues(root_pose_mat)
                mano_pose[:3] = root_pose.flatten()
                mano_pose[3:] -= rmano_pose_mean
                mano_shape = annot["handBeta"]
            else:
                mano_pose = None
                mano_shape = None
...
def process_sequence_batch(batch_seqs, worker_id):
    ...
    with wds.ShardWriter(worker_pattern, maxcount=MAX_COUNT, maxsize=MAX_SIZE) as sink:
        for cx, annots in enumerate(batch_seqs):
            ...
            # === 数据写入wds ===
            key_str = f"{worker_id}_{cx}_{SPLIT}_ho3d"
